...
if option=="Runtime (movie duration)": 
    ...
    figrm = px.scatter(movie_years, x="Year", y="IMDB_Rating", hover_data = ["Series_Title"])
...
if rating_options=="Runtime":
    ...
    figRV = px.scatter(movie_df, x="IMDB_Rating", y="No_of_Votes",color="Series_Title")
    ...
    figRR = px.scatter(movie_df, x="IMDB_Rating", y="Gross", color = "Series_Title", size ='IMDB_Rating', title= "Rating and Revenue Comparison")
...
st.title('Distribution of Genres')
...
st.caption("Takeaways from section #1")
